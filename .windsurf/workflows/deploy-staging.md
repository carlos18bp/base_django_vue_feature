...
```bash
cd /home/ryzepeck/webapps/base_django_vue_feature_staging && git fetch origin && git branch -r | grep $BRANCH
```
...
```bash
cd /home/ryzepeck/webapps/base_django_vue_feature_staging && git fetch origin && git checkout $BRANCH && git pull origin $BRANCH
```
