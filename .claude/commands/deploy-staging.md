...
Run these steps on the staging server at `/home/ryzepeck/webapps/base_django_vue_feature_staging` to deploy a release branch for client testing/UAT.

- **Domain**: https://gmconsultoresjuridicos.projectapp.co
- **Stack**: Django + Gunicorn + Nginx + MySQL 8 + Redis + Huey
- **Services**: `base_django_vue_feature_staging` (Gunicorn), `base_django_vue_feature-staging-huey` (task queue)
...
```bash
cd /home/ryzepeck/webapps/base_django_vue_feature_staging && git status
```
...
```bash
cd /home/ryzepeck/webapps/base_django_vue_feature_staging && git fetch origin && git branch -r | grep $ARGUMENTS
```
...
```bash
cd /home/ryzepeck/webapps/base_django_vue_feature_staging && git fetch origin && git checkout $ARGUMENTS && git pull origin $ARGUMENTS
```

5. Install backend dependencies and run migrations:
```bash
cd /home/ryzepeck/webapps/base_django_vue_feature_staging/backend && source venv/bin/activate && pip install -r requirements.txt && python manage.py migrate
```

6. Build the frontend (requires nvm for Node 22.13.0) and remove node_modules afterwards:
```bash
bash -c 'export NVM_DIR="$HOME/.nvm"; source "$NVM_DIR/nvm.sh"; nvm use; cd /home/ryzepeck/webapps/base_django_vue_feature_staging/frontend && npm ci && npm run build && rm -rf node_modules'
```
...
```bash
ls /home/ryzepeck/webapps/base_django_vue_feature_staging/frontend/node_modules 2>/dev/null && echo "WARNING: node_modules still present" || echo "OK: node_modules removed"
```

7. Collect static files:
```bash
cd /home/ryzepeck/webapps/base_django_vue_feature_staging/backend && source venv/bin/activate && python manage.py collectstatic --noinput
```
...
```bash
sudo systemctl restart base_django_vue_feature_staging && sudo systemctl restart base_django_vue_feature-staging-huey
```
...
```bash
sudo systemctl is-active base_django_vue_feature_staging && sudo systemctl is-active base_django_vue_feature-staging-huey
```
...
```bash
cd /home/ryzepeck/webapps/base_django_vue_feature_staging && git log --oneline -1
```
...
```bash
sudo journalctl -u base_django_vue_feature_staging --no-pager -n 50
```

13. Check Huey logs:
```bash
sudo journalctl -u base_django_vue_feature-staging-huey --no-pager -n 50
```
...
```bash
tail -50 /home/ryzepeck/webapps/base_django_vue_feature_staging/backend/debug.log
```

16. If services won't start, check systemd details:
```bash
sudo systemctl status base_django_vue_feature_staging --no-pager -l
sudo systemctl status base_django_vue_feature-staging-huey --no-pager -l
```
